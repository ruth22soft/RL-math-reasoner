...
generate_suffix() {
  ...
  while [[ "$#" -gt 0 ]]; do
    case $1 in
      ...
      --resume_mode) suffix+="_resume$2"; shift 2 ;;
      --mode) suffix+="_$2"; shift 2 ;;
      --fixed_beta) suffix+="_beta$2"; shift 2 ;;
      --rollout_gpu_memory_util) shift 2 ;;
      --rollout_max_num_seqs) shift 2 ;;
...
while [[ "$#" -gt 0 ]]; do
  ...
  case "$1" in
    ...
    --mode) TRAIN_MODE="$2"; shift 2 ;;
    --fixed_beta) FIXED_BETA="$2"; shift 2 ;;
...
if [[ "$RESUME_MODE" == "auto" ]]; then
  echo "resume_mode=auto is disallowed; forcing never"
  RESUME_MODE=never
fi
...
ray job submit --address=${HEAD_IP}:${HEAD_PORT} \
  ...
  "${EXTRA_TRAINER_ARGS[@]}" 2>&1 | tee -a $LOG_FILE_PATH
