...
CONTAINER_NAME="amharic-fixed-kl"
IMAGE_NAME="${IMAGE_NAME:-simple-rl:ngc-vllm}"
RUN_DIR="/ckpts/amharic_fixed_kl"
...
mkdir -p /ckpts/amharic_fixed_kl
PYTHONUNBUFFERED=1 python -m verl.trainer.main_ppo \
  --config-name simplelr_grpo_amharic_single_gpu \
  trainer.total_epochs=1 \
  trainer.default_local_dir=/ckpts/amharic_fixed_kl \
  ...
  actor_rollout_ref.actor.actor_adaptive_kl.mode=fixed \
  2>&1 | tee /ckpts/amharic_fixed_kl/train.log
'
