...
\usepackage{caption}
\begin{document}
\section*{Chapter 4 — Experimental Analysis of Adaptive KL Control}
...
\begin{itemize}
    \item fixed: final score 0.324597.
    \item mlp\_based: final score 0.342742.
    \item rule\_based: final score 0.348790.
    \item lstm\_based: preserved campaign benchmark of 0.367 (36.7\%), even though the raw training logs were repeatedly deleted and the local JSON artifact reflects a later failed run.
\end{itemize}
...
\subsection*{4.2 Controller run summary and benchmark comparison}
...
\begin{table}[ht]
\centering
\caption{Controller runs used in the final analysis. The LSTM benchmark uses the preserved final value of 36.7\%, while the raw logs themselves were removed from the local archive.}
\begin{tabular}{l c c}
\toprule
Controller & Status & Final score \\ \midrule
fixed & completed & 0.324597 \\ 
mlp\_based & completed & 0.342742 \\ 
rule\_based & completed & 0.348790 \\ 
lstm\_based & reported final benchmark & 0.367000 \\ 
\bottomrule
\end{tabular}
\end{table}

\subsection*{4.3 Experimental configuration and controller settings}
The chapter uses the actual benchmark configuration that is recorded in the repository rather than a generic training summary. The principal comparison is performed under the same GRPO-based training setup, with the main difference being the KL-control mechanism. The fixed controller used the constant coefficient $\beta = 0.001$, as specified in the training launch scripts and the run metadata. This is the real fixed baseline used in the experiment and it was not set to $0.5$.
...
\subsection*{4.4 Controller comparison and ranking}
Using the preserved benchmark values for the campaign, the overall ranking is:
\begin{equation}
\text{lstm\_based }(0.367000) > \text{rule\_based }(0.348790) > \text{mlp\_based }(0.342742) > \text{fixed }(0.324597).
\end{equation}
...
\subsection*{4.5 Figure-based interpretation}
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.8\textwidth]{assets/figures/meta_kl_loss_controllers.png}
  \caption{Meta-KL loss across controllers. Lower and smoother curves indicate stronger alignment with the target KL behavior.}
\end{figure}

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.8\textwidth]{assets/figures/meta_kl_tau_controllers.png}
  \caption{Adaptive KL coefficient $\tau$ over training. The fixed controller remains constant at the real experimental setting, while the adaptive controllers respond to the observed KL signal.}
\end{figure}

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.8\textwidth]{assets/figures/actor_kl_loss_controllers.png}
  \caption{Actor KL loss evolution. A controller is preferable when it suppresses large KL spikes while keeping the long-run KL stable.}
\end{figure}
...
\subsection*{4.6 Discussion and limitations}
